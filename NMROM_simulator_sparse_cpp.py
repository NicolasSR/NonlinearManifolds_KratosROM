    ...
    class CustomSimulation(cls):
        ...
        def __init__(self, model,project_parameters, custom_param = None, nn_rom_interface=None):
            ...
            if self.nn_rom_interface is None:
                ...
                self._GetSVDPhiMatrices=nn_rom_interface.get_phi_matrices
                self._GetNNLayers=nn_rom_interface.get_NN_layers
...
class NN_ROM_Interface():
    ...
    def get_phi_matrices(self):
        phi_inf, phi_sup = self.prepost_processor.get_phi_matrices()
        sigma_inf, sigma_sup = self.prepost_processor.get_sigma_vectors()
        phi_inf_weighted = phi_inf @ np.diag(sigma_inf)
        phi_sup_weighted = phi_sup @ np.diag(sigma_sup)
        return KratosMultiphysics.Matrix(phi_inf_weighted), KratosMultiphysics.Matrix(phi_sup_weighted)
    
    def get_NN_layers(self):
        layers=[]
        for layer in self.dec_network.trainable_variables:
