...
class W_Only_Strategy_KerasModel(keras.Model):
    ...
    def __init__(self, prepost_processor, kratos_simulation, strategy_config, *args, **kwargs):
        ...
        if strategy_config["r_loss_type"]=='norm':
            self.get_v_loss_w = self.kratos_simulation.get_v_loss_welemnorm
            self.get_err_w = self.kratos_simulation.get_err_welemnorm
        elif strategy_config["r_loss_type"]=='diff':
            self.get_v_loss_w = self.kratos_simulation.get_v_loss_welemdiff
            self.get_err_w = self.kratos_simulation.get_err_welemdiff
        else:
